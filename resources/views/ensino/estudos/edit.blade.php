...
@section('content')
<form action="{{ route('ensino.estudos.update', $estudo) }}" method="POST" enctype="multipart/form-data" id="studyForm">
    @csrf
    @method('PUT')
    <div class="row">
        <!-- Conteúdo Principal -->
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                               id="name" name="name" value="{{ old('name', $estudo->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="content" name="content" class="form-control @error('content') is-invalid @enderror" 
                                  style="display: none;">{{ old('content', $estudo->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body">
                    @if($estudo->featured_image)
                        <div id="featuredImagePreview" class="mb-3 text-center">
                            <img id="featuredImagePreviewImg" src="{{ asset('storage/' . $estudo->featured_image) }}" 
                        ...
                        <div id="featuredImagePlaceholder" class="text-center p-4 border rounded" 
                             style="background-color: #f8f9fa; min-height: 150px; display: none; align-items: center; justify-content: center;">
                            <div>
                    ...
                    @else
                        ...
                        <div id="featuredImagePlaceholder" class="text-center p-4 border rounded" 
                             style="background-color: #f8f9fa; min-height: 150px; display: flex; align-items: center; justify-content: center;">
                            <div>
                                <i class="bx bx-image" style="font-size: 48px; color: #6c757d; opacity: 0.3;"></i>
                                <p class="text-muted small mt-2 mb-0">Nenhuma imagem selecionada</p>
                            </div>
            ...
            <div class="card mb-3" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                ...
                <div class="card-body">
                    ...
                    @if($estudo->attachment)
                        <div id="attachmentExisting" class="mb-3">
                            <div class="alert alert-info d-flex justify-content-between align-items-center">
                                <span>
                                    <i class="bx bx-file me-1"></i>{{ $estudo->attachment_name ?? 'Arquivo anexado' }}
                                </span>
                                <a href="{{ asset('storage/' . $estudo->attachment) }}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
                                    <i class="bx bx-download"></i>
                                </a>
                                <button type="button" class="btn btn-sm btn-danger" onclick="removeExistingAttachment()">
                ...
                <div class="card-body">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="send_notification" 
                               ...
                               {{ old('send_notification', $estudo->send_notification) ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(old('content', $estudo->content))
            quill.root.innerHTML = {!! json_encode(old('content', $estudo->content)) !!};
        @endif
    ...
    function previewAttachment(input) {
        if (input.files && input.files[0]) {
            ...
            if (document.getElementById('attachmentExisting')) {
                document.getElementById('attachmentExisting').style.display = 'none';
            }
