@php
    ...
    $user = auth()->user();
    $canScheduleAny = $user && (
        $user->can('midia.instagram.schedule') || $user->can('midia.whatsapp.schedule')
    );
@endphp
<div class="dropdown midia-post-actions">
    ...
    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="{{ $menuId }}">
        ...
        @if($canRetryAny && $canScheduleAny)
            ...
            @endif
        @endif
        @if($post->canDeleteLocally() && $canScheduleAny)
            <li><hr class="dropdown-divider"></li>
            <li>
                <form method="POST" action="{{ route('midia.instagram.posts.destroy', $post) }}"
                      onsubmit="return confirm(@json($post->status === 'agendado' || $post->status === 'erro' ? 'Excluir este registro?' : 'Remover só o registro do ADELSS? A publicação no Instagram/WhatsApp (se existir) NÃO será apagada.'))">
                    @csrf
                    @method('DELETE')
