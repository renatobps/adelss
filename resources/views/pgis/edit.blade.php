...
@section('title', 'Editar PGI')

@section('page-title', 'Editar PGI')
    ...
    <div class="col-12">
        <section class="card">
            <header class="card-header">
                ...
                <h2 class="card-title">
                    <i class="bx bx-edit me-2"></i>Editar PGI
                </h2>
                <p class="card-subtitle">Edite as informações do PGI</p>
            </header>
            <div class="card-body">
                <form action="{{ route('pgis.update', $pgi) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                               id="name" name="name" value="{{ old('name', $pgi->name) }}" 
                                    ...
                                    <div class="mb-3">
                                        ...
                                        @if($pgi->logo_url)
                                            <div class="mb-2">
                                                ...
                                                <p class="small text-muted mt-1">Logo atual</p>
                                            </div>
                                        @endif
                                        ...
                                        @if($pgi->banner_url)
                                            <div class="mb-2">
                                                <img src="{{ asset('storage/' . $pgi->banner_url) }}" 
                                                     alt="Banner atual" 
                                                     style="max-width: 100%; max-height: 200px; border-radius: 4px; border: 2px solid #ddd;">
                                                <p class="small text-muted mt-1">Banner atual</p>
                                            </div>
                                        @endif
                                        <input type="file" class="form-control @error('banner') is-invalid @enderror" 
                                               id="banner" name="banner" accept="image/*">
                                        <small class="form-text text-muted">Deixe em branco para manter o banner atual. Formatos aceitos: JPEG, PNG, JPG, GIF, SVG. Tamanho máximo: 2MB.</small>
                                        @error('banner')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control @error('opening_date') is-invalid @enderror" 
                                               ...
                                               value="{{ old('opening_date', $pgi->opening_date ? $pgi->opening_date->format('Y-m-d') : '') }}">
                                        @error('opening_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('day_of_week') is-invalid @enderror" 
                                            ...
                                            <option value="segunda" {{ old('day_of_week', $pgi->day_of_week) == 'segunda' ? 'selected' : '' }}>Segunda-feira</option>
                                            <option value="terça" {{ old('day_of_week', $pgi->day_of_week) == 'terça' ? 'selected' : '' }}>Terça-feira</option>
                                            <option value="quarta" {{ old('day_of_week', $pgi->day_of_week) == 'quarta' ? 'selected' : '' }}>Quarta-feira</option>
                                            <option value="quinta" {{ old('day_of_week', $pgi->day_of_week) == 'quinta' ? 'selected' : '' }}>Quinta-feira</option>
                                            <option value="sexta" {{ old('day_of_week', $pgi->day_of_week) == 'sexta' ? 'selected' : '' }}>Sexta-feira</option>
                                            <option value="sábado" {{ old('day_of_week', $pgi->day_of_week) == 'sábado' ? 'selected' : '' }}>Sábado</option>
                                            <option value="domingo" {{ old('day_of_week', $pgi->day_of_week) == 'domingo' ? 'selected' : '' }}>Domingo</option>
                                        </select>
                                        @error('day_of_week')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('profile') is-invalid @enderror" 
                                            ...
                                            <option value="">Selecione...</option>
                                            <option value="Masculino" {{ old('profile', $pgi->profile) == 'Masculino' ? 'selected' : '' }}>Masculino</option>
                                            <option value="Feminino" {{ old('profile', $pgi->profile) == 'Feminino' ? 'selected' : '' }}>Feminino</option>
                                            <option value="Misto" {{ old('profile', $pgi->profile) == 'Misto' ? 'selected' : '' }}>Misto</option>
                                        </select>
                                        @error('profile')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('time_schedule') is-invalid @enderror" 
                                            ...
                                            <option value="">Selecione...</option>
                                            <option value="Manhã" {{ old('time_schedule', $pgi->time_schedule) == 'Manhã' ? 'selected' : '' }}>Manhã</option>
                                            <option value="Tarde" {{ old('time_schedule', $pgi->time_schedule) == 'Tarde' ? 'selected' : '' }}>Tarde</option>
                                            <option value="Noite" {{ old('time_schedule', $pgi->time_schedule) == 'Noite' ? 'selected' : '' }}>Noite</option>
                                        </select>
                                        @error('time_schedule')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('leader_1_id') is-invalid @enderror" 
                                            ...
                                            @foreach($members as $member)
                                                <option value="{{ $member->id }}" {{ old('leader_1_id', $pgi->leader_1_id) == $member->id ? 'selected' : '' }}>
                                                    {{ $member->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('leader_2_id') is-invalid @enderror" 
                                            ...
                                            <option value="">Nenhum</option>
                                            @foreach($members as $member)
                                                <option value="{{ $member->id }}" {{ old('leader_2_id', $pgi->leader_2_id) == $member->id ? 'selected' : '' }}>
                                                    {{ $member->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('leader_training_1_id') is-invalid @enderror" 
                                            ...
                                            <option value="">Nenhum</option>
                                            @foreach($members as $member)
                                                <option value="{{ $member->id }}" {{ old('leader_training_1_id', $pgi->leader_training_1_id) == $member->id ? 'selected' : '' }}>
                                                    {{ $member->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('leader_training_2_id') is-invalid @enderror" 
                                            ...
                                            <option value="">Nenhum</option>
                                            @foreach($members as $member)
                                                <option value="{{ $member->id }}" {{ old('leader_training_2_id', $pgi->leader_training_2_id) == $member->id ? 'selected' : '' }}>
                                                    {{ $member->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('address') is-invalid @enderror" 
                                               id="address" name="address" value="{{ old('address', $pgi->address) }}" 
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('neighborhood') is-invalid @enderror" 
                                               id="neighborhood" name="neighborhood" value="{{ old('neighborhood', $pgi->neighborhood) }}" 
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('notes') is-invalid @enderror" 
                                                  ...
                                                  placeholder="Digite observações sobre o PGI...">{{ old('notes', $pgi->notes) }}</textarea>
                                        @error('notes')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
