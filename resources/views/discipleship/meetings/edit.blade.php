...
<div class="row">
    <div class="col-lg-10">
        <div class="card" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <div class="card-body">
                <form action="{{ route('discipleship.meetings.update', $meeting) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('discipleship_member_id') is-invalid @enderror" id="discipleship_member_id" name="discipleship_member_id" required>
                            ...
                            @foreach($members as $m)
                                <option value="{{ $m->id }}" {{ old('discipleship_member_id', $meeting->discipleship_member_id) == $m->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control @error('data') is-invalid @enderror" 
                                   id="data" name="data" value="{{ old('data', $meeting->data->format('Y-m-d')) }}" required>
                            @error('data')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('tipo') is-invalid @enderror" id="tipo" name="tipo" required>
                                <option value="presencial" {{ old('tipo', $meeting->tipo) === 'presencial' ? 'selected' : '' }}>Presencial</option>
                                <option value="online" {{ old('tipo', $meeting->tipo) === 'online' ? 'selected' : '' }}>Online</option>
                            </select>
                            @error('tipo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('assuntos_tratados') is-invalid @enderror" 
                                  id="assuntos_tratados" name="assuntos_tratados" rows="4">{{ old('assuntos_tratados', $meeting->assuntos_tratados) }}</textarea>
                    ...
                    <div class="mb-3" id="goals-section">
                        <label class="form-label">Propósitos vinculados</label>
                        <small class="d-block text-muted mb-2">Selecione os propósitos discutidos neste encontro</small>
                        <div id="goals-container">
                            <p class="text-muted small" id="goals-placeholder">Carregando...</p>
                    ...
                    <div class="card mb-3" style="border-left: 4px solid #059669;">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('oracao_tempo_dia') is-invalid @enderror" id="oracao_tempo_dia" name="oracao_tempo_dia" required>
                                    @foreach([0,5,10,15,20,25,30,35,40,45,50,55,60] as $min)
                                        <option value="{{ $min }}" {{ old('oracao_tempo_dia', $meeting->oracao_tempo_dia) == (string)$min ? 'selected' : '' }}>{{ $min }} minutos</option>
                                    @endforeach
                                    <option value="mais_1h" {{ old('oracao_tempo_dia', $meeting->oracao_tempo_dia) === 'mais_1h' ? 'selected' : '' }}>+ de 1 hora</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('oracao_como_sao') is-invalid @enderror" id="oracao_como_sao" name="oracao_como_sao" rows="3" required>{{ old('oracao_como_sao', $meeting->oracao_como_sao) }}</textarea>
                            ...
                            <div class="mb-0">
                                ...
                                <textarea class="form-control @error('oracao_observacoes') is-invalid @enderror" id="oracao_observacoes" name="oracao_observacoes" rows="2">{{ old('oracao_observacoes', $meeting->oracao_observacoes) }}</textarea>
                    ...
                    <div class="card mb-3" style="border-left: 4px solid #dc2626;">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('jejum_horas_semana') is-invalid @enderror" id="jejum_horas_semana" name="jejum_horas_semana">
                                    @foreach([0,6,12,18,24] as $h)
                                        <option value="{{ $h }}" {{ old('jejum_horas_semana', $meeting->jejum_horas_semana) == (string)$h ? 'selected' : '' }}>{{ $h }} horas</option>
                                    @endforeach
                                    <option value="mais_24" {{ old('jejum_horas_semana', $meeting->jejum_horas_semana) === 'mais_24' ? 'selected' : '' }}>+ de 24 horas</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="jejum_tipo" id="jejum_total" value="total" {{ old('jejum_tipo', $meeting->jejum_tipo) === 'total' ? 'checked' : '' }} required>
                                        <label class="form-check-label" for="jejum_total">Total</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="jejum_tipo" id="jejum_parcial" value="parcial" {{ old('jejum_tipo', $meeting->jejum_tipo) === 'parcial' ? 'checked' : '' }}>
                                        <label class="form-check-label" for="jejum_parcial">Parcial</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="jejum_tipo" id="jejum_nenhum" value="nenhum" {{ old('jejum_tipo', $meeting->jejum_tipo ?? 'nenhum') === 'nenhum' ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="jejum_com_proposito" id="jejum_proposito_sim" value="sim" {{ old('jejum_com_proposito', $meeting->jejum_com_proposito) === 'sim' ? 'checked' : '' }} required>
                                        <label class="form-check-label" for="jejum_proposito_sim">Sim</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="jejum_com_proposito" id="jejum_proposito_nao" value="nao" {{ old('jejum_com_proposito', $meeting->jejum_com_proposito) === 'nao' ? 'checked' : '' }}>
                            ...
                            <div class="mb-0">
                                ...
                                <textarea class="form-control @error('jejum_observacoes') is-invalid @enderror" id="jejum_observacoes" name="jejum_observacoes" rows="2">{{ old('jejum_observacoes', $meeting->jejum_observacoes) }}</textarea>
                    ...
                    <div class="card mb-3" style="border-left: 4px solid #2563eb;">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('leitura_capitulos_dia') is-invalid @enderror" id="leitura_capitulos_dia" name="leitura_capitulos_dia" required>
                                    @foreach(range(0, 10) as $n)
                                        <option value="{{ $n }}" {{ old('leitura_capitulos_dia', $meeting->leitura_capitulos_dia) == (string)$n ? 'selected' : '' }}>{{ $n }} {{ $n == 1 ? 'capítulo' : 'capítulos' }}</option>
                                    @endforeach
                                    <option value="mais_10" {{ old('leitura_capitulos_dia', $meeting->leitura_capitulos_dia) === 'mais_10' ? 'selected' : '' }}>+ de 10</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="leitura_estuda" id="leitura_estuda_sim" value="sim" {{ old('leitura_estuda', $meeting->leitura_estuda) === 'sim' ? 'checked' : '' }} required>
                                        <label class="form-check-label" for="leitura_estuda_sim">Sim</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="leitura_estuda" id="leitura_estuda_nao" value="nao" {{ old('leitura_estuda', $meeting->leitura_estuda) === 'nao' ? 'checked' : '' }}>
                            ...
                            <div class="mb-0">
                                ...
                                <textarea class="form-control @error('leitura_observacoes') is-invalid @enderror" id="leitura_observacoes" name="leitura_observacoes" rows="2">{{ old('leitura_observacoes', $meeting->leitura_observacoes) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('proximo_passo') is-invalid @enderror" 
                                  id="proximo_passo" name="proximo_passo" rows="3">{{ old('proximo_passo', $meeting->proximo_passo) }}</textarea>
                        @error('proximo_passo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('observacoes_privadas') is-invalid @enderror" 
                                  id="observacoes_privadas" name="observacoes_privadas" rows="3">{{ old('observacoes_privadas', $meeting->observacoes_privadas) }}</textarea>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <a href="{{ route('discipleship.goals.create', ['discipleship_member_id' => $meeting->discipleship_member_id]) }}" class="btn btn-outline-success">
                            <i class="bx bx-target-lock me-1"></i>Cadastrar Propósito
                        </a>
                        <div class="d-flex gap-2">
                            <a href="{{ route('discipleship.meetings.show', $meeting) }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentGoalIds = @json(collect(old('goal_ids', $meeting->goals->pluck('id')->toArray()))->map(fn($i) => (string)$i)->values()->toArray());
        ...
        if (memberId && goalsByMember[memberId]) {
            ...
            if (goals.length > 0) {
                ...
                goals.forEach(function(g) {
                    const checked = currentGoalIds.includes(String(g.id)) ? ' checked' : '';
    ...
    if (memberSelect) {
        memberSelect.addEventListener('change', updateGoals);
        updateGoals();
    }
});
</script>
