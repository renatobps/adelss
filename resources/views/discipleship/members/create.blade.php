...
@section('title', 'Vincular Membro ao Ciclo')

@section('page-title', 'Vincular Membro ao Ciclo')
...
<div class="row">
    <div class="col-lg-8">
        <div class="card" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <div class="card-body">
                <form action="{{ route('discipleship.members.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('cycle_id') is-invalid @enderror" id="cycle_id" name="cycle_id" required>
                            ...
                            @foreach($cycles as $cycle)
                                <option value="{{ $cycle->id }}" {{ old('cycle_id', $cycleId) == $cycle->id ? 'selected' : '' }}>
                                    {{ $cycle->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('discipulador_id') is-invalid @enderror" id="discipulador_id" name="discipulador_id">
                            ...
                            @foreach($discipuladores as $discipulador)
                                <option value="{{ $discipulador->id }}" {{ old('discipulador_id') == $discipulador->id ? 'selected' : '' }}>
                                    {{ $discipulador->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control @error('data_inicio') is-invalid @enderror" 
                                   id="data_inicio" name="data_inicio" value="{{ old('data_inicio', date('Y-m-d')) }}" required>
                            @error('data_inicio')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('discipleship.members.index') }}" class="btn btn-secondary">
                            <i class="bx bx-x me-1"></i>Cancelar
                        </a>
                        <button type="submit" class="btn btn-primary">
                            <i class="bx bx-check me-1"></i>Vincular Membro
                        </button>
                    </div>
                </form>
            </div>
