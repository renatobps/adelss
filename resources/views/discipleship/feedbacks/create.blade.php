...
@section('page-title', 'Criar Feedback')
...
<div class="row">
    <div class="col-lg-8">
        <div class="card" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <div class="card-body">
                <form action="{{ route('discipleship.feedbacks.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('visibilidade') is-invalid @enderror" id="visibilidade" name="visibilidade" required>
                            <option value="discipulador" {{ old('visibilidade', 'discipulador') === 'discipulador' ? 'selected' : '' }}>Discipulador</option>
                            <option value="pastor" {{ old('visibilidade') === 'pastor' ? 'selected' : '' }}>Pastor</option>
                            <option value="admin" {{ old('visibilidade') === 'admin' ? 'selected' : '' }}>Admin</option>
                        </select>
                        @error('visibilidade')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('conteudo') is-invalid @enderror" 
                                  id="conteudo" name="conteudo" rows="6" required>{{ old('conteudo') }}</textarea>
                        @error('conteudo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="bx bx-check me-1"></i>Registrar Feedback
                        </button>
                    </div>
                </form>
            </div>
