...
@section('title', 'Nova Escala - Moriah')
    ...
    <div class="col-12">
        <section class="card">
            <header class="card-header">
                <h2 class="card-title">Nova escala</h2>
            </header>
            <div class="card-body">
                <form action="{{ route('moriah.schedules.store') }}" method="POST" id="scheduleForm">
                    @csrf
                    ...
                    <div class="tab-content" id="scheduleTabsContent">
                        ...
                        <div class="tab-pane fade show active" id="detalhes" role="tabpanel">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('event_id') is-invalid @enderror" id="event_id" name="event_id" required>
                                    ...
                                    @foreach($cultos as $culto)
                                        <option value="{{ $culto->id }}" data-title="{{ $culto->title }}" data-date="{{ $culto->start_date->format('Y-m-d') }}" data-time="{{ $culto->start_date->format('H:i') }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('observations') is-invalid @enderror" id="observations" name="observations" rows="3" maxlength="500">{{ old('observations') }}</textarea>
                                <small class="text-muted"><span id="observations-count">0</span>/500</small>
                                @error('observations')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="request_confirmation" name="request_confirmation" value="1" {{ old('request_confirmation') ? 'checked' : '' }}>
                                ...
                                @foreach($members as $member)
                                    <div class="member-item" data-name="{{ strtolower($member->name) }}" data-role="{{ strtolower($member->role?->name ?? '') }}">
                                        <div class="list-group-item d-flex align-items-center" style="background-color: {{ $member->moriahFunctions->count() > 1 ? '#f0e6ff' : 'transparent' }};">
                                            ...
                                            <div class="form-check form-switch">
                                                <input class="form-check-input member-checkbox" type="checkbox" name="members[]" value="{{ $member->id }}" id="member-{{ $member->id }}" data-member-id="{{ $member->id }}" data-functions-count="{{ $member->moriahFunctions->count() }}">
                                            </div>
                                        </div>
                                                ...
                                                <div class="functions-checklist" id="functions-checklist-{{ $member->id }}">
                                                    @foreach($member->moriahFunctions as $function)
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input function-checkbox" type="checkbox" name="member_functions[{{ $member->id }}][]" value="{{ $function->id }}" id="function-{{ $member->id }}-{{ $function->id }}" data-member-id="{{ $member->id }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Validar indisponibilidades antes de submeter
    ...
    if (scheduleForm) {
        scheduleForm.addEventListener('submit', function(e) {
            ...
            fetch('{{ route("moriah.unavailabilities.check") }}', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.has_unavailabilities && data.unavailable_members.length > 0) {
                    const memberNames = data.unavailable_members.map(m => m.member_name).join(', ');
                    const message = `Atenção! Os seguintes membros têm indisponibilidade cadastrada para esta data:\n\n${memberNames}\n\nDeseja continuar mesmo assim?`;
            ...
            .catch(error => {
                console.error('Erro ao verificar indisponibilidades:', error);
                // Em caso de erro, permitir submissão normal
                scheduleForm.submit();
            });
        });
    }
