...
@section('title', 'Relatório: Receitas e Despesas - Por categoria')
...
@section('breadcrumbs')
    ...
    <li><span>Receitas e Despesas - Por categoria</span></li>
@endsection
...
<div class="row">
    ...
    <div class="col-lg-9">
        ...
        <div class="card mb-4" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <div class="card-body">
                <form method="GET" action="{{ route('financial.reports.revenues-expenses.by-category') }}" id="filterForm">
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            <input type="text" class="form-control form-control-sm" placeholder="Descrição" name="search" value="{{ request('search') }}">
                        </div>
                        <div class="col-md-6 text-end">
                            ...
                            @if(request()->hasAny(['type', 'status', 'category_receitas_id', 'category_despesas_id', 'account_id', 'cost_center_id', 'start_date', 'end_date', 'search']))
                                <a href="{{ route('financial.reports.revenues-expenses.by-category') }}" class="btn btn-default btn-sm">
            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h6 class="mb-0">
                                Relatório: Receitas - Por categoria
                                @if($categoryReceitasId)
                                    ({{ $categoriesReceitas->firstWhere('id', $categoryReceitasId)->name ?? '' }})
                                @endif
                                @if($categoryDespesasId)
                                    / Despesas - Por categoria ({{ $categoriesDespesas->firstWhere('id', $categoryDespesasId)->name ?? '' }})
                                @endif
                            </h6>
                ...
                <div class="mb-4">
                    @include('financial.partials.daily-chart', [
                        ...
                        'area' => true,
                        'mobileNote' => 'Receitas e despesas agrupadas por período. Os previstos estão nas tabelas abaixo.',
                    ])
                </div>

                <!-- Seção Receitas -->
                @if($receitas->count() > 0 || !$categoryReceitasId)
                    <div class="mb-5">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="d-flex gap-2 align-items-center">
                                <form method="GET" action="{{ route('financial.reports.revenues-expenses.by-category') }}" id="tableFormReceitas" style="display: contents;">
                                    @foreach(request()->except(['search', 'per_page', 'receitas_page']) as $key => $value)
                                        @if(is_array($value))
                                            @foreach($value as $v)
                                                <input type="hidden" name="{{ $key }}[]" value="{{ $v }}">
                                            @endforeach
                                        @elseif($key !== 'despesas_page' && $key !== 'sort_by_despesas' && $key !== 'sort_order_despesas')
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead class="table-light">
                                        <tr>
                                            <th>
                                                <a href="{{ route('financial.reports.revenues-expenses.by-category', array_merge(request()->except(['sort_by', 'sort_order']), ['sort_by' => 'transaction_date', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" class="text-decoration-none text-dark">
                                                    Data
                                            ...
                                            <th>
                                                <a href="{{ route('financial.reports.revenues-expenses.by-category', array_merge(request()->except(['sort_by', 'sort_order']), ['sort_by' => 'description', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" class="text-decoration-none text-dark">
                                                    Descrição
                                            ...
                                            <th>
                                                <a href="{{ route('financial.reports.revenues-expenses.by-category', array_merge(request()->except(['sort_by', 'sort_order']), ['sort_by' => 'amount', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" class="text-decoration-none text-dark">
                                                    Total
                        ...
                        @else
                            <div class="text-center py-3 text-muted">
                                <p class="mb-0">Nenhuma receita encontrada para os filtros selecionados.</p>
                            </div>
                        @endif
                    </div>
                @endif
                ...
                <!-- Seção Despesas -->
                @if($despesas->count() > 0 || !$categoryDespesasId)
                    <div class="mb-5">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div></div>
                            <div class="d-flex gap-2 align-items-center">
                                <form method="GET" action="{{ route('financial.reports.revenues-expenses.by-category') }}" id="tableFormDespesas" style="display: contents;">
                                    @foreach(request()->except(['search', 'per_page', 'despesas_page']) as $key => $value)
                                        @if(is_array($value))
                                            @foreach($value as $v)
                                                <input type="hidden" name="{{ $key }}[]" value="{{ $v }}">
                                            @endforeach
                                        @elseif($key !== 'receitas_page' && $key !== 'sort_by_receitas' && $key !== 'sort_order_receitas')
                                            <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                        @endif
                                    @endforeach
                                    <select class="form-select form-select-sm" style="width: auto;" name="per_page" onchange="this.form.submit()">
                        ...
                        @else
                            <div class="text-center py-3 text-muted">
                                <p class="mb-0">Nenhuma despesa encontrada para os filtros selecionados.</p>
                            </div>
                        @endif
                    </div>
                @endif
