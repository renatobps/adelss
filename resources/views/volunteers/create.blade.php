...
@section('page-title', 'Novo Voluntário')
    ...
    <div class="col-12">
        <section class="card">
            <header class="card-header">
                ...
                <h2 class="card-title">
                    <i class="bx bx-user-plus me-2"></i>Novo Voluntário
                </h2>
            </header>
            <div class="card-body">
                <form action="{{ route('voluntarios.cadastro.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select @error('experience_level') is-invalid @enderror" 
                                ...
                                <option value="novo" {{ old('experience_level') == 'novo' ? 'selected' : '' }}>Novo</option>
                                <option value="em_treinamento" {{ old('experience_level') == 'em_treinamento' ? 'selected' : '' }}>Em Treinamento</option>
                                <option value="experiente" {{ old('experience_level') == 'experiente' ? 'selected' : '' }}>Experiente</option>
                            </select>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select class="form-select @error('service_areas') is-invalid @enderror" 
                                ...
                                @foreach($serviceAreas as $area)
                                    <option value="{{ $area->id }}" {{ in_array($area->id, old('service_areas', [])) ? 'selected' : '' }}>
                                        {{ $area->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea class="form-control @error('leader_notes') is-invalid @enderror" 
                                      ...
                                      placeholder="Observações sobre o voluntário...">{{ old('leader_notes') }}</textarea>
                            @error('leader_notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
